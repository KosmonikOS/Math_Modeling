...
class ThreeEdgeManipulatorForPlane:
    ...
    def findAngles(self,destX,destY):
        ...
        equation1 = self.r1 + self.r2 * sy.cos(φ2) + self.r3 * sy.cos(φ2 + φ3) - sy.sqrt(destX ** 2 + destY ** 2)
        firstResult = sy.solve(equation1,(φ2,φ3))
        if len(firstResult) == 0: return []
        φ2 = firstResult[0][0]
        equation2 = sy.cos(φ1) * (self.r1 + self.r2 * sy.cos(φ2) + self.r3 * sy.cos(φ2 + φ3)) - destX
